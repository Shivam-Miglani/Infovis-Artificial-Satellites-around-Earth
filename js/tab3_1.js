...
// Draw the bars
    svg.selectAll(".rect")
        ...
        .append('title')
        .text(function (d) {
            return "Number of satellites: " + d.value;
        });
...
function update3(Purpose, Users1) {
    ...
    var scByCountry1 = d3.nest()
        .key(function (d) {
            if (d.Purpose == Purpose && d.Users == Users1) {
                return d.Country;
            }
            else if (Users1 == "All") {
    ...
    console.log("scByCountry1:")
    console.log(JSON.stringify(scByCountry1));


    var scByCountry2 = scByCountry1;
    console.log(JSON.stringify(scByCountry2));
    var position = 0;
    for (var i in scByCountry1) {
        if (scByCountry1[i].value > 500) {
            position = i;
            break;
        }
    }
    console.log("country" + position);
    scByCountry2.splice(position, 1);
    console.log(JSON.stringify(scByCountry2));
    ...
    x.domain(scByCountry2.map(function (d) {
        if (d.value != 0)
            return d.key;
    }));
    y.domain([0, d3.max(scByCountry2, function (d) {
        return d.value;
    })]);
