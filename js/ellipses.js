...
function update1(Country, Users) {
    ...
    var xScale = d3.scale.linear()
        .domain([
            d3.min([0, d3.min(satellites, function (d) {
                return d.major
            })]),
            d3.max([0, d3.max(satellites, function (d) {
                return d.major
            })])
        ])
        .range([0, 50]);
    ...
    var ellipse = svg.selectAll('ellipse')
        ...
        .attr('cy', 312)
        .attr('rx', function (d) {
            if (d.Country == Country && d.Users == Users) {
                return xScale(d.major * 0.05)
            }
            else if (Country == "All") {
                if (d.Users == Users) {
                    return xScale(d.major * 0.05)
        ...
        .attr('stroke', function (d) {
            return colorScale(d.ClassofOrbit)
        })
        .attr("fill", "none")
        .attr('stroke-width', .5)
